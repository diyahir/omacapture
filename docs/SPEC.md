# Omacapture — Behavioral Specification

Omacapture is a Linux (Rust + GTK4, Wayland/Hyprland) screenshot and annotation tool. This document
...
| UserDefaults | TOML config (`~/.config/omacapture/config.toml`) + a small state file |
| NSPasteboard | `wl-clipboard` semantics via GTK4 `Gdk.Clipboard` (image/png + text/uri-list) |
| Carbon global hotkeys | Hyprland `bind` lines invoking `omacapture --capture area` (CLI/D-Bus), plus GlobalShortcuts portal where available |
| snapzy:// URL scheme | `omacapture` CLI subcommands and a D-Bus service; same verbs |
...
| Application Support | `$XDG_DATA_HOME/omacapture/` (captures cache, sessions, thumbnails, sqlite) |
...
Omacapture's own windows are hidden for the duration of a capture unless "include own windows" is on.
...
- **Confirm / cancel.**
  ...
  - `Enter` before dragging = "repeat last area" (see below). Omacapture also treats `Enter`/`Space`
...
- **Space-drag move.** Holding `Space` mid-drag moves the rectangle instead of resizing it (this is
  the behavior in the inline-annotate overlay; Omacapture applies it to plain area capture too).
- **Constraint modifiers (Omacapture addition — the reference app has none in the capture overlay).**
...
- **Window-pick mode (`A`).** Hovering highlights the topmost window under the pointer with a cutout
  ...
  where the compositor supplies them). Omacapture's own windows are never candidates. Press `A` again
...
Each after-capture action is an independent boolean, stored per capture kind. Omacapture has one kind
...
- **Cache folder** (when `save=off`): `$XDG_DATA_HOME/omacapture/captures/`. Deliberately not `/tmp`
...
Template string with tokens, default `Omacapture_{datetime}_{ms}`:
...
- **Watermark**: text; styles single / diagonal (rotated -24) / tiled (repeated, rotated -24);
  color, opacity, size, rotation. An image watermark is a Omacapture extension (drop an image onto the
  watermark tool).
...
Entry from the tray menu or `omacapture combine a.png b.png` (2+ files; fewer opens a picker). Modes:
...
- **Storage**: SQLite at `$XDG_DATA_HOME/omacapture/omacapture.db`. Each record: id, file path, kind
...
Path: `~/.config/omacapture/config.toml` (respect `$XDG_CONFIG_HOME`). Omacapture reads it at launch and
...
[capture.naming]
screenshot_template = "Omacapture_{datetime}_{ms}"

[capture.screenshot]
format = "png"                   # "png" | "jpg" | "webp"
jpeg_quality = 90                # Omacapture addition
webp_quality = 80                # Omacapture addition
...
remember_last_area = true        # Omacapture addition (reference always remembers)
...
languages = ["eng"]              # Omacapture addition: tesseract packs
...
Since Hyprland owns global keybinds, Omacapture also emits a ready-to-include snippet
(`~/.config/omacapture/hyprland.conf`) generated from `[shortcuts.global.*]`, e.g.
`bind = SUPER SHIFT, 3, exec, omacapture capture fullscreen`.
...
| Kind | Action | macOS reference | Omacapture default |
| --- | --- | --- | --- |
...
| `Shift` (hold) | Lock 1:1 (Omacapture addition) |
| Arrows / `Shift`+arrows | Nudge pending rect 1 / 10 px (Omacapture addition) |
...
`omacapture capture {fullscreen|area|repeat-area|window|active-window|area-annotate|ocr|object-cutout}`,
`omacapture open {annotate|history|combine <files...>}`, `omacapture show shortcuts`,
`omacapture settings [--page general|capture|annotate|quick-access|history|shortcuts|advanced|about]`.
...
4. Config file: explain `~/.config/omacapture/config.toml`, create it with defaults.
