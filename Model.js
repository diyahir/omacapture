...
// maps to an `omacapture` subcommand; the shell only ever launches the binary.
var BINARY = "omacapture"
...
var CANDIDATES = ["$HOME/.cargo/bin/omacapture", "$HOME/.local/bin/omacapture", "/usr/local/bin/omacapture", "/usr/bin/omacapture"]
...
var ACTIONS = [
  ...
  { id: "settings", icon: "",  label: "Preferences",           hint: "Omacapture settings",                 args: ["settings"] }
]
...
function probeScript() {
  ...
  return checks.join("; ") + '; command -v omacapture 2>/dev/null && exit 0; exit 1'
}
